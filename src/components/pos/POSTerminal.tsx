...
import { Search, X, Plus, Minus, Printer, Check } from "lucide-react";
...
interface Customer { id: number; name: string; phone?: string | null; creditLimit: unknown; }
...
export function POSTerminal({ products, customers }: Props) {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-gray-100">
        ...
        <div className="bg-white border-b p-3 flex gap-2 items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search product by name, code or variety..."
        ...
        <div className="flex-1 overflow-y-auto p-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 content-start">
          {filtered.map((p) => {
            ...
            return (
              <button
                key={p.id}
                onClick={() => dispatch({ type: "ADD_ITEM", product: { id: p.id, code: p.code, name: p.name, salePrice: p.salePrice, stockKg: p.stockKg, defaultUnit: p.defaultUnit as Unit } })}
                disabled={isOut}
              ...
              >
                ...
                <p className="text-xs text-gray-500">{p.variety}{p.brand ? ` • ${p.brand.name}` : ""}</p>
      ...
      {/* Right: Cart */}
      <div className="w-96 flex flex-col bg-white border-l overflow-hidden">
        {/* Customer picker */}
